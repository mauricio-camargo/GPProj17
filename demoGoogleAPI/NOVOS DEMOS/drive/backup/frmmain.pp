...
begin
  ...
  try
    ...
    Q.q:='mimeType = ''application/vnd.google-apps.folder'' and '''+AFolderId;
    Q.corpus:='';
//    q.maxResults:=0;
    Q.pageToken:='';
//    Q.projection:='';
      ...
      try
        if Assigned(List) then
          for i:= 0 to Length(List.files)-1 do
            ...
            Entry:=List.files[i];
            List.files[i]:=Nil;
            N:=AddChild(AParent,Entry.description);
            N.Data:=Entry;
            end;
      finally
        EndUpdate;
        ...
        if Assigned(List) then
          for i:= 0 to Length(List.files)-1 do
            ...
            With LI.SubItems do
              ...
              Add(Entry.size);
              Add(DateTimeToStr(Entry.modifiedByMeTime));
