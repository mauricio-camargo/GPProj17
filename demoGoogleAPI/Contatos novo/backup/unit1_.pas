...
begin
OpenUrl(AURL);
if InputQuery('var','vasd',S) then AAuthCode := S;
end;
...
begin
  ...
  TPeopleAPI.RegisterAPIResources;
...
var
  FPeopList: TListConnectionsResponse;
  FPerson: TPerson;

  //  Entry: TPerson;
  Resource: TPeopleResource;
  EN: string;
  i: integer;
begin
{  Memo3.Lines.Clear;
  FreeAndNil(FPeopList);
  FPeopList:=FPeopleAPI.PeopleConnectionsResource.List('people/me','');
  try
  FPeopList.connections.
    Resource:= FPeopleAPI.CreatePeopleResource;
  FPeopList := Resource.ConnectionsResource.List('','');
    SaveRefreshToken('Peop');
    if assigned(FPeopList) then
      for i := 0 to Length(FPeopList.) - 1 do
      begin
        FPerson := FPeopList.items[i];
        EN := FPerson.names;
        Memo3.Lines.Add(EN);
      end;
    Button7.Enabled := Memo3.Lines.Count > 0;
  finally
    FreeAndNil(Resource);
  end;
}
